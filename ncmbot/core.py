...
__all__ = [
    ...
    'music_comment', 'song_detail', 'personal_fm', 'user_record','toplist','user_info'
]
proxyHost = "http-dyn.abuyun.com"
#proxyHost = "proxy.abuyun.com"
proxyPort = "9020"
proxyUser = "H1OJ6077QDU2073D" #"H941185H9V92U03D"
proxyPass = "DFAD7E9C2A4C2C21" #"0B5117D9D8FABBD1"
...
class NCloudBot(object):
    ...
    _METHODS = {
        ...
        'TOPLIST': '/api/playlist/detail?id=%s',
    }
    ...
    def send(self):
        ...
        try:
            if self.method == 'SEARCH':
                ...
                resp = req.post(_url, data=self.data, proxies=proxies)
                ...
                if self.method in ('LYRIC', 'MUSIC_COMMENT','TOPLIST'):
                    _url = _url % self.params['id']
                # GET
                if self.method in ('LYRIC','TOPLIST'):
                    resp = req.get(_url, proxies = proxies)
                else:
                    resp = req.post(_url, data=data, proxies=proxies)
                self._build_response(resp)
                self.response.ok = True
